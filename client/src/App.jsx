...
function NavBar() {
  ...
  const moreLinks = [
    ...
    { to: '/name', label: '姓名测试', icon: '📝' },
    { to: '/sign', label: '抽签问卦', icon: '🙏' },
    ...
    <nav className="fixed top-0 left-0 right-0 z-50" style={{background: 'rgba(15, 5, 36, 0.9)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(217, 70, 239, 0.15)'}}>
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="text-lg font-bold bg-gradient-to-r from-purple-300 to-yellow-300 bg-clip-text text-transparent hidden sm:block">神秘占卜馆</span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-1">
          ...
          <div className="relative">
            <button onClick={() => setMoreOpen(!moreOpen)}
              ...
              更多 <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7"/></svg>
            </button>
            {moreOpen && (
              <>
